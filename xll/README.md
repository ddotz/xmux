...
Once loaded, each Excel window owns its own CTP, bridge, workbook context, and selection feed. The add-in subscribes to Excel `AppEvents` `WindowActivate` and `WindowDeactivate` callbacks, queues one coalesced reconciliation at the Excel macro boundary, and performs an initial queued scan; it does not poll `Application.Windows`. A dirty generation and in-flight gate drain events that arrive during reconciliation, while a bounded macro-context retry retains dirty state after a transient failure. A pane entry retains and compares its COM window identity as well as its HWND, so a reused HWND disposes the old pane instead of binding it to a different Excel window. The add-in never releases Excel-DNA's shared Application or Window RCWs: it unadvises event handlers and clears managed references, while only balanced `IUnknown` identity probes are released. Lifecycle callback failures are contained and reported, and incomplete event unsubscription or pane disposal remains retryable during close.

Excel's `SheetSelectionChange` event queues the latest `{kind, address, worksheetId}` and delivers it after a 25 ms settle window (and mouse release for a drag), so range reads do not race Excel's in-progress selection gesture. Each pane filters the application-wide event by its owning window handle. The subscription starts only after the pane has registered its selection handlers and completed the matching `context.sync()`, and the current selection is queued once at startup.
...
- Leave the workbook idle, switch sheets and windows, activate a chart sheet, then close and reopen windows (including a rapid close/reopen that reuses a window handle); verify the initial scan and each activation create the correct pane, deactivation removes closed panes, no `Application.Windows` polling or repeated selection COM reads occur, and there is no cross-window delivery, callback to a disposed pane, or stale subscription.
