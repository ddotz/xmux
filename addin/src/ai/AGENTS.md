...
- `taskpane/chatting.ts` rejects the obsolete workbook proposal shape and asks for validated direct tools. Skill proposals still wait for `onSaveSkill`. Every changed write batch refreshes the workbook snapshot, and a final answer is held until a post-write read checks headers, first/last rows, formulas and omissions. The pane appends its own exact execution ledger; model prose is never the sole evidence that a write landed.
