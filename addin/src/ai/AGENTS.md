...
- **The server validates message structure.** `conversationFor` keeps one system message first, drops empty turns, merges consecutive assistant status lines, and keeps only the newest consecutive user request so an abandoned failed command is never replayed beside its correction. Compaction summaries stay explicitly quoted at user priority and are never promoted into system instructions.
...
- Reply parsing checks `finish_reason`, refusal, and non-empty visible content before anything reaches the tool parser. `length`/filter/refusal/thinking-only replies are non-executable errors, even when their content begins with valid tool JSON.
...
- A parsed `edits`/`blocks`/`newSheets` proposal is never applied: `planTouchesWorkbook` only detects it so `chatting.ts` can tell the model to run real tools instead. A `skill` proposal is length-bounded (name ≤64, label ≤80, description ≤240, instructions ≤4000, ≤20 triggers) and is the only proposal the pane still renders.
- **Tool calls are the only workbook-write path.** `readSteps` takes the last fenced block, else every JSON span (widest first, 12 max). The complete capped batch validates before any call runs: a malformed element, overflow, or unknown/misspelled field rejects the whole batch atomically, preventing a corrected retry from repeating a non-idempotent prefix. Unfenced truncated calls are rejected, never printed.
- **Workbook facts have a read-side trust boundary too.** Before final prose that claims an A1 value/blank/zero/formula/error reaches the user, `taskpane/chat-grounding.ts` exact-reads the cited cells (including addresses carried only by the request) and `chatting.ts` asks for one rewrite from those newest observations. Large selection-wide claims require every deterministic tile; sampling, truncated tile output, new unchecked citations, or insufficient round/context capacity yields an explicit unknown answer.
...
- `taskpane/chatting.ts` rejects the obsolete workbook proposal shape and asks for validated direct tools. It normalises qualified addresses (`sheet` + `Sheet!A1` becomes one local target), refreshes after changed batches, and holds the final answer until a post-write read checks headers, first/last rows, formulas and omissions. Failed attempts disappear from the final ledger only when a later successful call recovers the same destination and effect; unresolved failures remain explicit.
