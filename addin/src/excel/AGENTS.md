...
| `host-bridge.ts` | 5 | **The non-WEF wire.** The pane's half of an in-process host (XLL + WebView2 host object): accessors and loads become a generic op list, `sync` sends it in one call, the response is the only thing a read can come from. That makes the load/sync protocol structural — an unloaded property has nothing to read — and it is what the `.NET` side is implemented against. Eight members plus `load`. |
| `bridge-memory.ts` | 3 | The other side of that wire, in memory: the reference implementation of the dispatch table a host object owes. Test-only, ships in `src/`, never loaded by the pane. |
