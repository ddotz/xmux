...
| `bridge-memory.ts` | 3 | The other side of that wire, in memory: the reference implementation of the dispatch table a host object owes. Seventeen members implemented — `worksheets`, `getItem`, `getItemOrNullObject`, `getActiveWorksheet`, `add`, `getRange`, `getUsedRange`, `getNameRange`, `getTableRange`, `getSelectedRange`, `func`, `set` (formulas, numberFormat, `format.*`), `insert`, `delete`, `clear`, `autoFill` — and the rest fail at `sync` with `bridge: no dispatch for "sort" — the host object still owes this member`. That message *is* the remaining C# backlog; a member that goes silent instead is a bug. **Ops execute in issue order, mutations included** — a load after a write in the same batch reads the new value, because that is what Office answers and a host that reorders answers a different question. Test-only, ships in `src/`, never loaded by the pane. |
