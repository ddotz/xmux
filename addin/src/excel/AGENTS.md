...
| `host-office.ts` | 2 | The Office.js implementation of that port, and the only module that may touch `Excel`/`Office`/`OfficeExtension` at runtime. Thin forwards, one structural cast, no pane logic. |
| `host-xll.ts` | 3 | The **second** implementation: `ExcelHost` over the bridge, for a pane hosted by an in-process XLL's WebView2. Owns the host-object path `window.chrome.webview.hostObjects.xmux` — named here and nowhere else, because it is a contract with the C# side. |
| `host-entry.ts` | 1 | `startHost` picks between them, and is the only module that knows both exist. Selection is a **positive** test for the injected host object: "is `Office` defined yet" is a race, since office.js loads before its own handshake. |
| `host-bridge.ts` | 5 | **The non-WEF wire.** The pane's half of an in-process host (XLL + WebView2 host object): accessors, property assignments and loads become a generic op list, `sync` sends it in one call, the response is the only thing a read can come from. That makes the load/sync protocol structural — an unloaded property has nothing to read — and it is what the `.NET` side is implemented against. `context satisfies OperateContext` — by assignability, never by cast, which `host.test.ts` enforces: a cast once made the façade *look* complete while most of the write surface was absent, and an uncovered tool died on `undefined` naming nothing. Two op kinds only (`call`, `load`); `set` is the single generic property write and nested ones are dotted paths (`set("format.fill.color", …)`). A new pane feature adds a **member**, never an op kind. |
| `bridge-memory.ts` | 3 | The other side of that wire, in memory: the reference implementation of the dispatch table a host object owes. Every member the pane's tools reach is implemented or recorded, each one exercised by a real `runWrite`/`runDataTool` call over the wire — a dispatch entry no tool reaches is a guess about the C# side. A member the host does *not* have fails at `sync` with `bridge: no dispatch for "…" — the host object still owes this member`, and a test issues an op for a member nobody will ever implement to keep that guarantee true independently of what happens to be covered. **Recorded, not modelled**: charts, pivots, frozen panes, conditional formats, filters, protection and page layout have no meaning in a grid of display strings, so the host records the call and its arguments and refuses to *answer* a read about them — a fixture that answered would be inventing Excel's behaviour, and the C# side is written against what this host says. A table and its range are **separate handles** (`getTable` then `getRange`), because a handle that is both teaches the host a confusion it would implement faithfully. That message *is* the remaining C# backlog; a member that goes silent instead is a bug. **Ops execute in issue order, mutations included** — a load after a write in the same batch reads the new value, because that is what Office answers and a host that reorders answers a different question. Test-only, ships in `src/`, never loaded by the pane. |
...
## WHAT THE `.NET` SIDE OWES

The pane's whole demand on a non-WEF host, settled on a Mac so a Windows session implements a list instead of designing one. `bridge-memory.ts` is the reference implementation of all of it.

- `handshake()` returning `{ workbookUrl, capabilities: [{ name, version }] }`. **Neither lazy nor optional**: `isSetSupported` and `workbookUrl` are *synchronous* on the port and a bridge cannot make a round trip synchronously, so both answers must already be in hand before the pane asks. An undeclared capability answers `false` rather than guessing `true`; an unknown workbook answers `""` rather than inventing a URL.
- `execute(ops)` returning `{ values }`, or `{ values, failure: { code, message } }` when an op was refused. A refusal is **data, not prose** — it arrives as `BridgeHostError` carrying the code, and `classify` asks with `instanceof`. Recovering a code by splitting an error message turns the pane's own protocol violations into fake Excel errors shown to the user.
- `cellEditMode` as a failure code. Excel refuses every API call while a cell editor is open, on COM exactly as on Office.js, and the pane's degrade-in-place behaviour hangs on recognising it. A host that reports it as an ordinary error blanks the pane over something the user does constantly.
- Selection **pushed**, not polled: registration is a call op, delivery is a callback carrying `{ address, worksheetId }`. Selection is the pane's only trigger, so a host without this is a pane that never updates.
...
- **Load what you read, read after you sync.** Office is lenient about the first and silent about the second, so neither mistake shows up against the ordinary fakes (`load: () => {}` over a populated object). A read consumer belongs in `host-bridge.test.ts`, where both are refused because the response physically lacks the value: dropping `items/visibility` from the `listSheets` load passes every other test in the repo and fails there. The write path has no equivalent yet — extending the op list over `operate.ts` is the first thing the XLL track needs.
- A new pane feature adds a **member to the bridge dispatch table**, never a change to the wire. If you find yourself adding an op kind, the generic `call` is being worked around.
