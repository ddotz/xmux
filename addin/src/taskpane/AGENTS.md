...
**chat**: the AI tab.
- `chatting.ts` the state machine: prompt assembly, `askModel`, `parsePlan`, apply-through-`recordWrite`, settings + local skill persistence to `localStorage`. `chat.ts`/`chat-controls.ts`/`chat-settings.ts`/`chat-skill-ui.ts`/`chat-icons.ts` render only. `chat-context.ts` + `chat-prompt.ts` + `chat-skills.ts` + `chat-skill-store.ts` pure. `chat-workbook.ts` takes an injected `Excel.RequestContext` and reads a 9x7 neighborhood.
...
- Never make the chat apply edits automatically. `chat-prompt.ts` pins `writes: "proposal-only"`; the user presses 적용.
