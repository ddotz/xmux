...
**chat**: the AI tab.
- `chatting.ts` the state machine: prompt assembly, `askModel`, the tool loop (`readSteps` → `runTool`/`runWrite`, up to `MAX_TOOL_ROUNDS`), `parsePlan` as the fallback shape, settings + local skill persistence to `localStorage`. Every budget comes from `budgetFor(state.settings)` — the configured window, not a constant — and they keep a long build inside what the server takes: `boundRound` shares `roundChars` out over one round's results, fair-share so a small answer beside a huge one survives whole; `trimObservations` carries the newest result whole and older ones only while `observationChars` and `keptObservations` both allow; `compactTurns` folds the thread past `carriedTurns`. On the 128k window in use that is a 1,629-cell read and 78,000 characters of results carried; on 32k it is the old 500 cells and 13,000. An answer that only *announces* work (`announcesWork`) is not accepted as the end of the turn: it goes back once with an act-now instruction, and a second promise stands rather than spending the user's rounds. A batch repeated verbatim is **not** run again (`insert_rows` twice is not `insert_rows` once) — the model is told, and a third identical batch ends the tool phase. `withReceipt` is what reaches the screen: `plainText` off the markdown, the pane's own list of calls when the model answered with nothing, and one line about undo when a call ran that the history cannot take back. `chat.ts`/`chat-controls.ts`/`chat-settings.ts`/`chat-skill-ui.ts`/`chat-icons.ts` render only. `chat-context.ts` + `chat-prompt.ts` + `chat-skills.ts` + `chat-skill-store.ts` pure. `chat-workbook.ts` takes an injected `Excel.RequestContext` and reads a 9x7 neighborhood.
...
- Never put a raw model reply on screen. It passes `containsToolCall` (JSON is a conversation with the pane, not an answer) and `plainText` (`**`, `#`, backticks and fences are punctuation in a `pre-wrap` bubble) before it becomes a turn.
