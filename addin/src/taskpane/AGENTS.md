...
- `markdown.ts` parses CommonMark/GFM to an AST and builds an allowlisted DOM tree with `createElement`/text nodes only. Raw HTML and images never execute or fetch. HTTP(S) links are privacy-hardened; reported `Sheet!A1:B9` and bare A1 ranges are buttons that navigate through `main.ts`'s guarded command path.
