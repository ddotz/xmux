...
| 7 | Trusted Catalog(공유 폴더) 채널 | **실패** — 2026-09-01 실기기 파일럿 보고 | `v1.13.2-catalog.1` 실행 결과; 실패 단계 상세 캡처는 미회수 | 사용자 단위 Trusted Catalog도 이 환경의 WEF 취득을 복구하지 못함. `main` 병합 금지, 비WEF 채널로 전환 |
...
2026-09-01 파일럿에서 케이스 7이 실패했으므로 아래 실패 분기가 실제 판정이다.
...
| 2.5 | 어휘·프로토콜 명시와 검증 — 열거형 단어 17개 유니온 + `WordsFit` parity, load/sync 5개 조항, 그걸 **강제하는 컬텍스트**로 읽기 소비자 전수 통과 | 불필요 | **완료** |
...
아니라 `main`에 있다. 단계 3·4는 파일럿이 실패했을 때만 착수한다.
...
2. **프로토콜이 계약에도, 테스트에도 없었다.** `HostContext`는 동기 API가 아니라 **지연 객체 그래프**다:
   ...
   sync 전에 읽어도 전부 통과**했다. 그래서 `strict-context.ts`를 넣었다 — 읽기 계약을
   구현하되 세 가지를 거부한다(미로드 읽기, sync 전 읽기, 배치 종료 후 핸들 사용).
   `resolveReferences`·`summariseReferences`·`listSheets`·`readWindow`를 그걸로 돌렸고 전부
   통과했다 — 이제 "패인의 읽기 경로는 포팅 가능하다"는 가정이 아니라 결과다. 검증:
   `sheets.ts`의 로드에서 `items/visibility`를 빼면 저장소의 다른 모든 테스트는 그대로
   통과하고 이 스위트만 정확히 그 지점을 짚는다. 쓰기 경로(`operate.ts`)는 아직 미검증이고,
   단계 3이 시작되면 그게 첫 번째 할 일이다.
...
Catalog도 결국 **WEF 채널**이다. 이 환경에서 WEF 취득은 케이스 1·3·4·7이 실패,
5가 무의미하고 6은 세션 한정 완화책(24h TTL 미검증)이다. 즉 **지속적으로 동작하는
WEF 취득 경로가 없다.**
...
- **Windows PC 없이 할 수 있는 XLL 준비를 먼저 끝낸다.** 패인 쪽 절반은 전부 맥에서
  ...
  통과하기 전까지는 미검증 베팅이므로 `main`을 오염시키지 않는다.
...
- [x] Trusted Catalog 파일럿 `setup` → 첫 추가 판정: **실패** (2026-09-01 조작자 보고; 상세 캡처 미회수)
