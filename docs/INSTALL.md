...
### 추가 기능 채널 전환 (Trusted Catalog 실험)

이 채널은 Office 첫 추가 시 발생하는 오류를 근본적으로 피할 수 있는지 확인하기 위한
**실험 채널**이며 파일럿 검증 전입니다. `땡땡엑셀 설치.bat` 메뉴에서
**6. 추가 기능 채널 전환 (Trusted Catalog 실험)**을 선택합니다.

- **사내 공유(UNC)로 전환**: 회사의 공유 폴더 경로를 사용합니다. 관리자 권한이
  필요하지 않습니다.
- **이 PC에 로컬 공유 만들기**: 이 PC에 공유 폴더를 만들며 관리자 승인 1회가
  필요합니다.

전환한 뒤 Excel을 완전히 종료했다가 다시 실행하고
**삽입 > 추가 기능 > 내 추가 기능 > 공유 폴더 > 땡땡엑셀 > 추가**를 선택합니다.
Developer 채널로 돌아가려면 6번 메뉴의 **Developer 채널로 복귀**를 선택합니다.
문제가 생기면 같은 메뉴의 **채널 상태** 출력과 함께 담당자에게 보고합니다.
